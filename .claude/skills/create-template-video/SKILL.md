...
- `frame-aicoding-list` — **DANH SÁCH** 2–5 mục (mỗi mục có icon + tag mức độ), nền tối gradient.
- `frame-aicoding-comparison` — **SO SÁNH 2 thứ** (cũ vs mới, A vs B) — 2 card khung gradient + badge WIN + stat.
- `frame-gradient-quote` — **CÂU NÓI VIRAL / NHẬN ĐỊNH MẠNH** — glassmorphism card + gradient nền + orbs. Slots: quote, author, tag.
- `frame-split-reveal` — **SO SÁNH TRỰC QUAN** (trước/sau, đúng/sai) — 2 panel trượt vào từ 2 bên + VS badge. Slots: left_label, left_text, left_icon, right_label, right_text, right_icon, divider_text.
- `frame-neon-callout` — **HIGHLIGHT Ý CHÍNH / MẸO HAY** — card viền neon pulsing + icon lớn. Slots: icon, title, body, footer.
- `frame-timeline-step` — **HƯỚNG DẪN TỪNG BƯỚC / QUY TRÌNH** — timeline dọc + dots phát sáng. Slots: title, kicker, steps[{label, desc, status}].
- `frame-magazine-feature` — **EDITORIAL TRANG BÌA** — ảnh nền full-bleed + headline serif + pull-quote. Slots: kicker, headline, pull_quote, credit. Cần `visual.imagePrompt` hoặc `visual.searchQuery`.

**OUTRO:**
- `frame-image-outro` — **outro mặc định** / end-card thương hiệu (ảnh nền + sticker + tên brand + CTA). Cần `visual.searchQuery` (hoặc `visual.imagePrompt`) và tuỳ chọn `visual.iconQuery`.
...
- scenes[0].type = `hook`; scene cuối .type = `outro` (templateId = `frame-image-outro`).
...
- **body → Claude TỰ CHỌN template hợp nhất cho từng scene** theo nội dung (ưu tiên ĐA DẠNG — đừng lặp 1 template cho mọi body). Chọn trong:
    ...
    - `frame-gradient-quote` — **câu nói/nhận định viral** (quote, wisdom, philosophy). Card kính mờ + nền gradient động + orbs. Slots: quote, author, tag.
    - `frame-split-reveal` — **so sánh trực quan** (trước/sau, đúng/sai, pros/cons) KHÔNG cần số liệu. 2 panel slide từ 2 bên + VS badge. Slots: left_label, left_text, left_icon, right_label, right_text, right_icon, divider_text.
    - `frame-neon-callout` — **highlight ý chính, mẹo hay, "bạn có biết"** — card neon viền phát sáng. Slots: icon (emoji), title, body, footer.
    - `frame-timeline-step` — **hướng dẫn từng bước, quy trình, lộ trình** — timeline dọc + dots phát sáng. Slots: title, kicker, steps[{label, desc, status}]. status = done/active/pending.
    - `frame-magazine-feature` — **editorial trang bìa** — ảnh nền full-bleed (cần `visual.imagePrompt`/`visual.searchQuery`) + headline serif + pull-quote. Slots: kicker, headline, pull_quote, credit. Hợp cho review, lifestyle, giới thiệu chủ đề.
- outro → `frame-image-outro` (mặc định; slots: brand, cta + `visual.searchQuery`/`imagePrompt`, tuỳ chọn `visual.iconQuery`). Dùng `frame-statement-outro` (slots: cta, channel, source) nếu muốn card đỏ nền giấy.
...
- **`inputs` (chữ HIỂN THỊ trên màn hình) ĐƯỢC PHÉP dùng emoji/icon** để sinh động (🔥 🚀 ✨ ⚡ 📈 ⚠️ → …) — render màu OK. Giữ định dạng số đẹp ("5.5", "82%"). Tách biệt hoàn toàn với voiceText.
  - **SMART DESIGN SKILL**: 
    - Dùng emoji INLINE **rất tiết chế** (0–1 icon mỗi field, đặt ở nhãn/kicker/badge/CTA ngắn — vd kicker "🔥 Tin nóng", cta "Theo dõi ngay →"). 
    - TUYỆT ĐỐI KHÔNG nhét emoji vào các trường text typography lớn (như `figure`, chữ `hero` của build-minimal, hoặc `headline` 1-2 từ) vì sẽ phá vỡ thiết kế chuyên nghiệp.
    - Với các template có hỗ trợ `visual.iconQuery` (như `frame-image-hero`, `frame-visual-body`), hãy dùng từ khoá tiếng Anh (vd: "robot", "fire", "rocket") để kích hoạt hệ thống **3D Fluent Emoji**. Đây là cách chèn sticker sang trọng và đẹp nhất, thay vì dùng emoji dạng text.
