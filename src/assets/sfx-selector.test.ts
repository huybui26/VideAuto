...
describe("pickSfxForScene", () => {
  // Tier 3 (template default) and tier 4 (any-category fallback) are
  // deliberately disabled in sfx-selector.ts: firing an SFX on every scene made
  // the videos noisy. Only an explicit override (handled by the pipeline) or a
  // semantic keyword match produces a sound now.
  it("returns null when no keyword matches, even if the index is full", () => {
  ...
  it("returns deterministic result for same scene id", () => {
    const r1 = pickSfxForScene({
      voiceText: "Cảnh báo: sự cố lớn.", templateName: "feature-list",
  ...
  it("returns DIFFERENT files for different scene ids (variety)", () => {
    ...
    // At least 2 unique picks across 5 scenes (alert pool has 2 files)
    const unique = new Set(picks);
    expect(unique.size).toBeGreaterThanOrEqual(2);
  });

  it("returns null when the matched category is missing from the index", () => {
    const sparseIndex: SfxIndex = { outro: ["tada.mp3"] }; // no alert/ category
    const r = pickSfxForScene({
      voiceText: "Cảnh báo: sự cố lớn.",
      templateName: "hook",
      sceneId: "hook",
      index: sparseIndex,
    });
