...
export class OmniVoiceClient implements TtsClient {
  ...
  async generate(text: string, audioOutPath: string, _srtOutPath?: string, seed?: number, instruct?: string): Promise<void> {
    ...
    for (let attempt = 0; attempt < 4; attempt++) {
      try {
        ...
        if (instruct !== undefined) payload.instruct = instruct;
