...
**Tuỳ chọn — Telegram bot** (`npm run bot`). Bot render ngay trên máy bạn nên
**mặc định đóng**: chỉ những Telegram user ID có trong `ALLOWED_TELEGRAM_IDS`
mới dùng được, để trống là chặn tất cả. Lấy ID qua [@userinfobot](https://t.me/userinfobot).

```env
TELEGRAM_BOT_TOKEN=123456:ABC...
ALLOWED_TELEGRAM_IDS=111111111,222222222
GEMINI_API_KEY=...   # /render — Gemini viết script.json
PEXELS_API_KEY=...   # tuỳ chọn — ảnh stock cho visual layer
```
...
| `frame-image-outro`         |  outro  | End-card mặc định — ảnh nền + tên brand + CTA         |
